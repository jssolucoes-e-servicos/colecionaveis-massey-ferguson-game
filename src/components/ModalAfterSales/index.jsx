...
const ModelAfterSales = ({
  ...
  function handleAudioEffectLogin() {
    let audiosales = document.querySelector("#audioSales");
    audiosales.volume = 1.0;
    audiosales.play();
  }
  var confetti = {
  ...
  return (
    <ModalSale id={id} onClick={HandleOutsideClick}>
      <audio
        id="audioSales"
        autoPlay
      ...
      <ModelSaleF>
        <ModalSalesG>
          <React.Fragment>
            ...
            {data.type === "out" ? (
              <React.Fragment>
                <SalesTxts>{data.message}</SalesTxts>
                <SalesTxts>
                  <Translator path="components.new_cash" />
                  {` M$ ${data.cash}`}
                </SalesTxts>
              </React.Fragment>
            ) : (
...
export default ModelAfterSales;
